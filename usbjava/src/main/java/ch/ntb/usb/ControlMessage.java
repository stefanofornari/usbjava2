...
final public class ControlMessage
{
	...
	byte		buf [] = new byte [len];
        ...
        dev.controlMsg(
                (byte)(DIR_TO_HOST | descriptorClass | RECIPIENT_DEVICE),
                GET_DESCRIPTOR,
                (short) ((descriptorType << 8) | (0xff & id)),
                (short) index,
                buf,
                len,
	...
	dev.controlMsg(
            (byte)(DIR_TO_DEVICE | descriptorClass | RECIPIENT_DEVICE),
            SET_DESCRIPTOR,
            (short) ((descriptorType << 8) | (0xff & id)),
            (short) index,
            buf,
	...
	try {
		...
		(byte)UsbDescriptor.USB_DT_STRING,
		(byte) 0, 0, 256);
	...
	if (id == 0)
	    throw new IllegalArgumentException ();
	buf = getStandardDescriptor (dev, (byte)UsbDescriptor.USB_DT_STRING,
		id, language, 256);

	if (buf.length < 2
		|| buf [1] != UsbDescriptor.USB_DT_STRING
